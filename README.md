...
| 라이브러리 | 버전 | 선택 이유 |
|---|---|---|
| React | ^19.2.6 | 컴포넌트 기반 UI 프레임워크 |
| TypeScript | ~6.0.2 | 타입 안정성 및 개발 생산성 향상 |
| TanStack Query | ^5.100.11 | 서버 상태(캐싱·로딩·에러) 관리. useState만으로 구현 시 로딩·에러·재시도 처리가 중복 코드로 증가 |
| Recharts | ^3.8.1 | 선언적 React 컴포넌트 기반 차트. 주간 요약 시각화를 최소한의 코드로 구현 |
...
├── mocks/                 # MSW 핸들러·Mock 데이터 스토어 (localStorage 영속화)
└── test/                  # 테스트 설정
```
...
`PLANNER_SLOT_MINUTES = 30` (30분 단위)
...
- **활용**: 기능 설계, 컴포넌트 구조 설계, 유틸 함수 구현, CSS 스타일링, 테스트 작성, 문서 보완
